  ...
  <page class="h100">
    ...
    <view class="scroll-wrapper">
      <scroll-view scroll-y style="height: 100%; width: 100%" @scrolltolower="handleScrollToLower">
        <view class="plr-l">
          <view class="tip-text">下方为选中节次均为空闲的本年级老师</view>
...
import { getFreeTeachersForAgent } from '../services/curriculum-adjust';
...
function fetchData(loadMore?: boolean) {
  ...
  searching.value = setTimeout(async () => {
    ...
    if (loadMore) {
      data.pageNum = pageNum.value + 1;
    } else {
      data.pageNum = 1;
    }
    try {
      loading.value = true;
      const res: any = await getFreeTeachersForAgent(data);
      loading.value = false;
...
function getCellIds(list: any[]) {
  const res: any[] = [];
  list.forEach(i => {
    i.courseCells.forEach(j => {
      res.push({
        cellId: j.id,
        clazzDate: j.clazzDate,
      });
    });
  });
...
onMounted(() => {
  ...
  if (instance) {
    ...
    eventChannel.on('acceptDataFromOpenerPage', function (data: any) {
      prevData.value = data;
      cells.value = getCellIds(data.sourceCourse);
...
function handleSelect(item: any) {
  uni.navigateTo({
    url: '/app-school-affairs/curriculum-supply/apply-3',
