  ...
  <view class="page">
    <view class="main">
      ...
      <AssetSelectedAndScanList
        ...
        :api-query="{
          assetStatuses: [AssetStatus.Borrow],
          recipientIdList: [formState?.returnUserId],
        }"
        :order-type="RecordTypeEnum.ReturnRecord"
...
import {
  assetBorrowReturn,
  getAssetBorrowReturnDetail,
} from '@/app-general-affairs-logistics/assets-manage/services/back';
import {
  AssetStatus,
  RecordTypeEnum,
...
onLoad((options: any) => {
  ...
  if (options.assetId && options.assetId === assetDetailToCreateStore.assetInfo.assetId) {
    ...
    if (_asset.recipientId) {
      formState.value.returnUserId = _asset.recipientId;
      formState.value.returnUserName = _asset.recipientName;
    }
...
watch(
  ...
  newVal => {
    if (newVal) {
      formState.value.returnDate = dayjs(newVal.returnDate).format('YYYY-MM-DD');
      formState.value.returnUserName = newVal.returnUserName;
      formState.value.returnUserId = newVal.returnUserId;
      formState.value.deptId = newVal.deptId;
      formState.value.deptName = newVal.deptName;
      formState.value.preBackTime = dayjs(newVal.preBackTime).format('YYYY-MM-DD');
      formState.value.spaceId = newVal.spaceId;
      formState.value.spaceName = newVal.spaceName;
      formState.value.placeAfterReturn = newVal.placeAfterReturn;
      ...
      itemDTOList.value = newVal.assetItemList.map(item => {
        return {
          assetInfoImgVOList: item.assetInfoImgVOList,
          id: item.id,
...
const change = (e: any) => {
  formState.value.returnDate = e.result;
};
...
const handleSelectPersons = () => {
  ...
  showSelector({
    ...
    multiple: false,
    value: formState.value.returnUserId,
    callback: (value, data) => {
      formState.value.returnUserId = value || '';
      formState.value.returnUserName = data?.name || '';
      itemDTOList.value = [];
      if (!value) {
...
const handleToList = () => {
  ...
  if (type === 'home') {
    uni.redirectTo({
      url: `/app-general-affairs-logistics/assets-manage/pages/approve/index?tab=${RecordTypeEnum.ReturnRecord}`,
    });
  } else {
    uni.navigateBack({
      delta: 1,
...
const submit = async () => {
  ...
  if (!formState.value.returnUserId) {
    uni.showToast({
      title: '请选择借用人',
      icon: 'none',
    });
  ...
  formState.value.itemList = itemDTOList.value.map(item => {
    return {
  ...
  const params = {
    ...omit(formState.value, ['returnUserName', 'deptName', 'spaceName']),
  };
  uni.showLoading();
  loading.value = true;
  assetBorrowReturn(params)
    .then(res => {
      uni.hideLoading();
      uni.showToast({
        title: '借用成功',
