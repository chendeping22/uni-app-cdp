...
import { getAssetBorrowReturnDetail } from '@/app-general-affairs-logistics/assets-manage/services/back';
...
const getDetail = async () => {
  showLoading();
  getAssetBorrowReturnDetail(pageParams.value?.orderId)
    .then(res => {
      console.log(res);
...
const resubmit = () => {
  uni.redirectTo({
    url: `/app-general-affairs-logistics/assets-manage/pages/back/index?id=${detailData.value.fId}`,
  });
};
</script>
<template>
  <page custom-overflow="visible">
    ...
    >
      ...
      <view class="detail-text">
        ...
        <view class="main-item">
          <view class="main-item-title">归还后所在空间</view>
          <view class="main-item-content">{{
            detailData?.spaceName ? detailData?.spaceName : '/'
          }}</view>
        </view>
        <view class="main-item">
          <view class="main-item-title">归还后存放位置</view>
          <view class="main-item-content">{{
            detailData?.placeAfterReturn ? detailData?.placeAfterReturn : '/'
          }}</view>
        </view>
        <view class="main-item">
          <view class="main-item-title">归还备注</view>
          <view class="main-item-content">{{
      ...
      <asset-detail-list :assetList="detailData?.assetItemList" />
      <approve-flow
        ref="approveFlowRef"
