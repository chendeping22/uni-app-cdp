    ...
    <u-card :border="false" :show-head="false" :show-foot="false" @click="onClick"
      ><template #body>
        ...
        <view v-for="(item, index) in contentData" :key="index" class="list-card-item"
          ><ListCardItem :item-data="item" :index="headerData?.value + index"
        /></view>
      </template>
    </u-card>
  </view>
...
const props = defineProps({
  ...
  column: {
    ...
    type: Array as PropType<
      { label: string; key: string; dataRender: (e?: any, record?: any) => {} }[]
    >,
    default: [],
  },
  statusList: {
...
const contentData = computed(() =>
  slice(props?.column, 1)?.map(v => {
    ...
    const formatVal = v?.dataRender ? v?.dataRender(val, props?.cardData) : val;
    return { ...v, value: formatVal || '/' };
  }),
);

const flowStatus = computed(() => props.cardData?.[props?.statusFileName]);
