...
function calCategoriesData(categories, opts, config, eachSpacing, context) {
  ...
  if (opts.xAxis.rotateLabel == true) {
    ...
    tempHeight =
      tempHeight < fontSize + 2 * config.xAxisTextPadding
        ? tempHeight + 2 * config.xAxisTextPadding
        : tempHeight;
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    if (item.showLabel) {
      ...
      context.fillText(
        String(labelText),
        textX,
        bgStartY + config.fontSize + config.toolTipPadding / 2,
      );
      context.stroke();
      context.setTextAlign('left');
    }
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        if (item !== null && i > leftNum && i < rightNum) {
          ...
          if (
            ...
            const width = item.width;
            const height =
              opts.height - opts.area[2] - item.y - (mountOption.borderWidth * opts.pix) / 2;
      ...
      for (let i = 0; i < points.length; i++) {
        ...
        if (item !== null && i > leftNum && i < rightNum) {
          ...
          context.quadraticCurveTo(
            item.x - 0,
            opts.height - opts.area[2] - height / 4,
            item.x,
            item.y,
          );
          context.quadraticCurveTo(
