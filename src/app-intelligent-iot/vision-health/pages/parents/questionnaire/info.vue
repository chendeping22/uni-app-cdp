<template>
  <view class="">
    <view class="questionnaire-bg">
      <image
        class="zy-width-full"
        src="@/app-intelligent-iot/static/image/img_questionnaire.png"
        mode="widthFix"
      ></image>
...
const getQuestionDetails = () => {
  $http.questionaire
    ...
    .then(res => {
      pageData.quesList.forEach(item => {
        let index = res.result.questions.findIndex(i => i.id == item.id);
        if (index > -1) {
          if (item.type == '1') {
            const answer = res.result.questions[index].answer;
            ...
            item.answerContent = res.result.questions[index].answer || '未填写';
          } else if (item.type == '3') {
            const answer = res.result.questions[index].answer;
