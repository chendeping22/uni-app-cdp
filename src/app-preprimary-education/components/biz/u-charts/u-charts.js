...
/**
 * hhj-2023/4/18  新增当前最新版本markLine label配置属性
 */
...
function drawToolTip(textList, offset, opts, config, context, eachSpacing, xAxisPoints) {
  ...
  if (toolTipOption.toolTipArr.length > 0) {
    textList = toolTipOption.toolTipArr[opts.tooltip.index];
  }
  ...
  textList.forEach(function (item, index) {
    var startX =
      offset.x + arrowWidth + 2 * config.toolTipPadding + legendWidth + legendMarginRight;
    if (isOverRightBorder) {
      startX =
        offset.x -
        toolTipWidth -
          ...
          for (let i = 0; i < points.length; i++) {
            ...
            if (item !== null && i > leftNum && i < rightNum) {
              ...
              if (columnOption.barBorderCircle) {
                ...
                context.arc(
                  startX + item.width / 2,
                  item.y + barBorderRadius,
                  barBorderRadius,
                  -Math.PI,
                  0,
                );
                context.lineTo(
                  startX + item.width - columnOption.meterBorder,
                  opts.height - opts.area[2],
                );
                context.lineTo(startX, opts.height - opts.area[2]);
                context.fill();
              ...
              if (columnOption.meterBorder > 0) {
                ...
                if (columnOption.barBorderCircle) {
                  ...
                  context.moveTo(
                    startX + columnOption.meterBorder * 0.5,
                    opts.height - opts.area[2],
                  );
                  context.lineTo(startX + columnOption.meterBorder * 0.5, item.y + barBorderRadius);
                  context.arc(
                    startX + item.width / 2,
                    item.y + barBorderRadius - columnOption.meterBorder * 0.5,
                    barBorderRadius,
                    -Math.PI,
                    0,
                  );
          ...
          for (let i = 0; i < points.length; i++) {
            ...
            if (item !== null && i > leftNum && i < rightNum) {
              ...
              if (columnOption.barBorderCircle) {
                ...
                context.arc(
                  startX + item.width - barBorderRadius,
                  item.y + barBorderRadius,
                  barBorderRadius,
                  -Math.PI / 2,
                  0,
    ...
    for (let i = 0; i < series.length; i++) {
      if (i == series.length - 1) {
        ...
        if (funnelOption.linearType == 'custom') {
          ...
          grd.addColorStop(
            0.5,
            hexToRgb(funnelOption.customColor[series[i].linearIndex], funnelOption.fillOpacity),
          );
