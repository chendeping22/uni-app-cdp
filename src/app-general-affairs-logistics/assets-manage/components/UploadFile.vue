...
import { getFileIcon } from '@/app-general-affairs-logistics/components/attachments-list/attachment.ts';
...
const fileUploadResultWX = files => {
  const _file = files?.[0];
  if (_file) {
    fileList.value.push({
      fileId: _file.fileId,
...
const _handleFileAddWX = () => {
  callBridge({
    service: Service.file,
    action: Action.fileUploadByH5,
    data: {
      fileCount: 1,
...
const _handleFileAdd = async () => {
  ...
  file = await callBridge({
    ...
    data: { fileFilter: 'pdf/*,doc/*,xls/*,txt/*,ppt/*,image/*,video/*,' },
  }).catch((err: any) => {
    const { code, msg } = err;
  ...
  await new Promise((resolve, reject) => {
    chooseFile({
      ...
      extension: ['.pdf', '.doc', 'docx', '.xls', '.xlsx', '.txt', '.wps'],
...
onMounted(() => {
  // #ifdef MP-WEIXIN
  uni.$on('fileUploadResult', fileUploadResultWX);
  // #endif
});

onUnmounted(() => {
  // #ifdef MP-WEIXIN
  uni.$off('fileUploadResult', fileUploadResultWX);
  // #endif
});
</script>
...
<template>
  <view class="upload-file">
    <view class="upload-file-head"
      ...
      <u-button
        style="margin: 0"
        :type="fileList.length === props.maxCount ? 'defalut' : 'primary'"
    ...
    <view v-if="fileList.length" class="file-list">
      ...
      >
        ...
        <view class="file-item-delete" @click.stop="handleFileDelete(file)"> 删除 </view>
      </view>
    </view>
  </view>
</template>
