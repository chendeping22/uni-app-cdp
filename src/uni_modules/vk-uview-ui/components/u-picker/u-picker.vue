...
export default {
	...
	methods: {
		...
		// 生成递进的数组
		generateArray: function(start, end, bool) {
			// 转为数值格式，否则用户给end-year等传递字符串值时，下面的end+1会导致字符串拼接，而不是相加
			start = Number(start);
			end = Number(end);
			end = end > start ? end : start;
			const num = bool ? this.step : 1;
			// 生成数组，获取其中的索引，并剪出来
			return [...Array(end + 1).keys()].slice(start).filter(v => v % num  === 0);
		},
		...
		initTimeValue() {
			...
			this.minutes = this.generateArray(0, 59, true);
			this.minute = this.minutes[parseInt(time.getMinutes() / this.step)];
