<template>
  <view class="page">
    <view v-if="detailData" class="main">
      <view class="head">
        <view class="head-top"
          ><view class="head-title">资产退还</view>
          <view
      ...
      <view class="detail-text">
        <view class="main-item">
          <view class="main-item-title">退还日期</view>
        ...
        <view class="main-item">
          ...
          <view class="main-item-content">{{ detailData?.returnUserName }}</view>
        </view>
        <view class="main-item">
        ...
        <view class="main-item">
          <view class="main-item-title">退还后所在空间</view>
          <view class="main-item-content">{{
            detailData?.spaceName ? detailData?.spaceName : '/'
          }}</view>
        </view>
        <view class="main-item">
          <view class="main-item-title">退还后存放位置</view>
          <view class="main-item-content">{{
            detailData?.placeAfterReturn ? detailData?.placeAfterReturn : '/'
          }}</view>
        </view>
        <view class="main-item">
          <view class="main-item-title">退还备注</view>
          <view class="main-item-content">{{
      ...
      <asset-detail-list :assetList="detailData?.assetAcceptanceReturnAssetInfoList" />
      <approve-flow
        ref="approveFlowRef"
...
import { getAssetAcceptanceReturnDetails } from '@/app-general-affairs-logistics/assets-manage/services/sendBack';
...
const getDetail = async () => {
  showLoading();
  getAssetAcceptanceReturnDetails(pageParams.value?.orderId || '')
    .then((res: any) => {
...
const resubmit = () => {
  uni.redirectTo({
    url: `/app-general-affairs-logistics/assets-manage/pages/sendBack/index?id=${detailData.value?.fId}`,
  });
};
